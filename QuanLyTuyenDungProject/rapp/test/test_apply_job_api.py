...
def test_api_apply_job_success(test_client, mocker):
    ...
    # Kiểm tra xem DAO có được gọi với đúng tham số không
...
# 3. TEST XEM DANH SÁCH ĐÃ NỘP (GET)
def test_api_get_my_applications(test_client, mocker):
    ...
    # Giả lập danh sách trả về từ DAO
    ...
    # Endpoint giả định cho trang cá nhân
...
# 4. TEST BẢO MẬT: EMPLOYER KHÔNG ĐƯỢC NỘP ĐƠN
def test_api_apply_job_unauthorized(test_client, mocker):
    class FakeUser():
        is_authenticated = True
        id = 5
        user_role = UserRole.EMPLOYER  # Role sai
